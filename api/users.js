...
//设置RTMP
router.post('/rtmpset', async (ctx) => {
    ctx.status = 200
    user = ctx.session.user
    const time=moment().add(24, 'h').format('YYYY-MM-DD HH:mm:ss')
    const timetmp=moment(time).unix()
    const server = 'localhost'
    const HashValue = md5(`/live/${user}-${timetmp}-''`)
    const rtmpkey = `${user}?sign=${timetmp}-${HashValue}`
    const hls = `http://${server}:8443/live/${user}/index.m3u8`
    await User.updateOne({
        user: user
    }, {
            $set: {
