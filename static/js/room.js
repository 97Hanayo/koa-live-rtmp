...
$(function () {
    ...
    const qrtmp = $('#qrtmp').qrcode({ width: 125, height: 125, text: window.location.href }).hide()
    const qrcode = qrtmp.find('canvas').get(0)
    const img = qrcode.toDataURL('image/jpeg')
    $("#qr").attr("data-original-title", `<img src=${img}>`)
})
